...
- 상품 텍스트와 메타 데이터는 `src/data/products.json` 또는 data 폴더 내 JSON을 사용한다.
- 제품 이미지는 상품 데이터의 image 필드 또는 연결된 경로 기준으로 사용한다.
- 제품 외 이미지/영상/배경/보조 비주얼은 `src/assets`와 `public/images`를 직접 사용한다.
- 즉, **상품 데이터와 에디토리얼 비주얼은 분리 관리**한다.
