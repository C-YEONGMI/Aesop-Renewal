import { listData } from '../api/data.js';
...
const mainPage = () => {
    ...
    const services = document.querySelectorAll('.con2 .service');
    const indicator = document.querySelector('.con2 .indicator');
    const currentCount = document.querySelector('.con2 #current-count');
    const serviceImg = document.querySelector('.con2 .service-img');
    const listDatas = document.querySelector('.con2 .service-copy p');
    const serviceHeight = 60;
    const imgHeight = 250;

    listDatas.textContent = listData[0][0];
    let currentSplitText = new SplitType(listDatas, { types: 'lines' });
  ...
  `;
        ...
        return new Promise((resolve) => {
            gsap.to(currentSplitText.lines, {
                ...
                onComplete: () => {
                    ...
                    const newText = listData[index][0];
                    listDatas.textContent = newText;

                    currentSplitText = new SplitType(listDatas, {
                        types: 'lines',
                    });
    ...
    ScrollTrigger.create({
        ...
        onUpdate: async (self) => {
            ...
            gsap.set('.con2  .progress', { scaleY: progress });
